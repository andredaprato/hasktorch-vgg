...
type BatchSize = 4
forward :: forall dtype device batchSize .
           _ =>
           Linear 4096 10 dtype device ->
           Vgg16 dtype device ->
           Tensor device dtype [batchSize, 3, 224, 224 ] ->
           Tensor device dtype [batchSize, 10]
...
imageTransform =  normalize . upsample_bilinear2d @224 @224 False . expand @'[BatchSize, 3, 28, 28] False . reshape @'[BatchSize, 1, 28, 28]
main = do
